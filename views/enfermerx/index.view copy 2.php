            ...
            <main class="flex-1 overflow-y-auto bg-gray-50 p-4 md:p-6 w-full">
                <div class="bg-white rounded-lg shadow-sm p-6">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
                        <div>
                            <h2 class="text-xl font-semibold text-gray-800">Lista de Enfermerx</h2>
                            <p class="text-sm text-gray-500 mt-1">Gestiona los Enfermerx del sistema</p>
                        </div>
                        <button class="mt-4 md:mt-0 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-md text-sm font-medium transition duration-150 ease-in-out">
                            <i class="fas fa-plus mr-2"></i>Nuevo Enfermerx
                        </button>
                    </div>
                    
                    <?php 
                    // Obtener los doctor del controlador
                    $enfermerx = $enfermerx ?? ($this->get('enfermerx') ?? []);
                    ...
                    <div class="overflow-x-auto">
                        <?php if (empty($enfermerx)): ?>
                            <div class="text-center py-4 text-gray-500">
                                No hay Enfermerx para mostrar.
                            </div>
                        <?php else: ?>
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Enfermerx</th>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <?php foreach ($enfermerx as $enfermerx): 
                                        $nombreCompleto = trim(($enfermerx['Nombres'] ?? '') . ' ' . ($enfermerx['Apellidos'] ?? ''));
                                        $iniciales = '';
                                    ...
                                    ?>
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                <?php echo htmlspecialchars($enfermerx['Id_Usuario'] ?? ''); ?>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="flex items-center">
                                                    ...
                                                    <div class="text-sm font-medium text-gray-900">
                                                        <?php echo htmlspecialchars($enfermerx['Nombre'] ?? 'Sin nombre'); ?>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                <?php echo htmlspecialchars($enfermerx['Email'] ?? 'No especificado'); ?>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                <?php 
                                                if (!empty($enfermerx['Edad'])) {
                                                    echo htmlspecialchars($enfermerx['Edad']) . ' años';
                                                } else {
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                <?php 
                                                if (!empty($enfermerx['Altura'])) {
                                                    echo htmlspecialchars($enfermerx['Altura']) . ' cm';
                                                } else {
                                                    echo 'No especificado';
                                                }
                                                ?>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                <?php 
                                                echo !empty($enfermerx['Tipo_sangre']) 
                                                    ? htmlspecialchars($enfermerx['Tipo_sangre'])
                                                    : 'No especificado';
                                                ?>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                <?php 
                                                if (!empty($enfermerx['Fecha_Registro'])) {
                                                    $fecha = new DateTime($enfermerx['Fecha_Registro']);
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <a href="<?php echo URL; ?>enfermerx/ver/<?php echo $enfermerx['Id_Usuario']; ?>" class="text-emerald-600 hover:text-emerald-900 mr-3" title="Ver">
                                                    <i class="far fa-eye"></i>
                                                </a>
                                                <a href="<?php echo URL; ?>enfermerx/editar/<?php echo $enfermerx['Id_Usuario']; ?>" class="text-blue-600 hover:text-blue-900 mr-3" title="Editar">
                                                    <i class="far fa-edit"></i>
                                                </a>
                                                <a href="<?php echo URL; ?>enfermerx/eliminar/<?php echo $enfermerx['Id_Usuario']; ?>" class="text-red-600 hover:text-red-900" title="Eliminar" onclick="return confirm('¿Estás seguro de eliminar este paciente?')">
